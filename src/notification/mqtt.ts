import {Brand, Link, Model, Store} from '../store/model'
...
export function sendMqttMessage(link: Link, store: Store): void {
  if (client) {
...
function generateTopic(link: Link, store: Store, topic: string): string {
  ...
  topic = topic
    ...
    .replace(/%brand%/g, link.brand as Brand)
    .replace(/%model%/g, link.model as Model)
